<?php if (isset($arena)) : ?>
    ...
    for ($x = 0; $x < $arena->getSize(); $x++) : ?>
        <?php for ($y = 0; $y < $arena->getSize(); $y++) : ?>
            <div>
                <?php foreach ($arena->getMonsters() as $fighter) : ?>
                    <?php if ($fighter->getX() === $x && $fighter->getY() === $y) : ?>
                        <img
                         title="Distance to Héraclès = <?= method_exists($arena, 'getDistance') ? $arena->getDistance($arena->getHero(), $fighter) : '' ?>" 
                         class="monster <?= method_exists($arena, 'touchable') && $arena->touchable($arena->getHero(), $fighter)  ? 'touchable' : 'untouchable' ?>" 
                         src="<?= $fighter->getImage(); ?>" 
                         alt=""
                        >
                    <?php endif; ?>
                <?php endforeach; ?>
